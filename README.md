...
| 9 | **All BambooHR text treated as untrusted.** Every result is wrapped in a labelled envelope that tells the model the content is data, not instructions. Free-text fields (notes, job titles, descriptions, file names, comments) are additionally wrapped in `[UNTRUSTED TEXT FROM BAMBOOHR ...]` markers, and control characters are stripped. | `src/policy.ts`, `src/tools/shared.ts` |
| 10 | **Supply chain pinned.** Releases are built by GitHub Actions from a version tag, signed with Sigstore (keyless, bound to this repository), attested with SLSA build provenance and published with SHA-256 checksums. Third-party actions are pinned to commit SHAs. The bundle is invoked by absolute path from a fixed install directory; there is no `npx -y` and no dependency resolution at runtime. At start-up the server fetches a version revocation list and refuses to run if its version is revoked. | `.github/workflows/release.yml`, `src/selfCheck.ts` |
...
- Bump `version` in `package.json` and `manifest.json`, add a `CHANGELOG.md` section, tag `vX.Y.Z` and push the tag. The release workflow builds, signs and publishes the bundle. `npm run bundle` builds it locally for testing.
...
| `bamboohr_time_off_requests` | Requests overlapping a range, with filters. Sick leave shown as `absent` without notes. Capped. |
...
| `bamboohr_changed_employees` | Employee ids inserted, updated or deleted since a timestamp, newest first. |
