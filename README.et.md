...
- Uue versiooni väljaandmiseks tõsta `version` failides `package.json` ja `manifest.json`, lisa `CHANGELOG.md`-sse jaotis, loo silt `vX.Y.Z` ja lükka see üles. Väljalasketöövoog ehitab, allkirjastab ja avaldab paketi. `npm run bundle` ehitab kohapeal testimiseks allkirjastamata koopia; see on teadlikult git-ignoreeritud.
